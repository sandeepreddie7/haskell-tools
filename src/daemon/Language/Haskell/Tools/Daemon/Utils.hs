...
lookupModuleColl moduleName = find (any ((moduleName ==) . (^. sfkModuleName)) . Map.keys . (_mcModules))
...
lookupSourceFileColl fp = find (any ((fp ==) . (^. sfkFileName)) . Map.keys . (_mcModules))
...
lookupModuleInSCs moduleName = find ((moduleName ==) . (^. sfkModuleName) . fst) . concatMap (Map.assocs . (_mcModules))
...
lookupModInSCs key = find (((key ^. sfkFileName) ==) . (^. sfkFileName) . fst) . concatMap (Map.assocs . (_mcModules))
...
lookupSFKInSCs key = listToMaybe . catMaybes . map (Map.lookup key . (_mcModules))
...
removeModule moduleName = map (\x -> x{_mcModules = Map.filterWithKey (\k _ -> moduleName /= (k ^. sfkModuleName)) (_mcModules x)})
...
codeGeneratedFor key codeGen = map (\x -> x{_mcModules = Map.adjust (\y -> y{_modRecCodeGen = codeGen}) (sfkFileName .= "" $ key) 
                                 . Map.adjust (\x -> x{_modRecCodeGen = codeGen}) key $ _mcModules x})
...
isAlreadyLoaded key codeGen = maybe False (\(_, mc) -> isLoaded mc && (Just $ _modRecCodeGen mc) < Just codeGen)
                                . find ((key ==) . fst) . concatMap (Map.assocs . (_mcModules))
...
insertIfMissing sfk mods
  = case lookupSFKInSCs sfk mods of
      Just _ -> mods
      Nothing -> element 0 .- (\x -> x{_mcModules = Map.insert sfk (ModuleNotLoaded NoCodeGen False) $ _mcModules x}) $ mods
