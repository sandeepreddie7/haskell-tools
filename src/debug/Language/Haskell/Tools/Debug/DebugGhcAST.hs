...
{-# LANGUAGE UndecidableInstances, TypeFamilies, DataKinds #-}
...
-- import Constraint (WantedConstraints(..), Ct(..), Implication(..), CtEvidence(..))
...
-- import Outputable (Outputable(..), showSDocUnsafe, SDoc(..))
-- import PatSyn (PatSyn(..))
-- import RdrName (RdrName(..), LocalRdrEnv(..))
-- import TcEvidence
-- import TcType
...
-- deriving instance Show (FamEqn GhcPs (HsDataDefn GhcPs))
...
-- deriving instance Show (FamEqn GhcPs (LHsType GhcPs))
...
-- -- deriving instance Show (HsConPatDetails (GhcPass pass)) 
...
-- deriving instance Show (HsRecFields GhcPs (Located (Pat (GhcPass 'Parsed))))
-- deriving instance Show (HsMatchContext RdrName)
-- deriving instance Show (HsConDetails (Located (Pat (GhcPass 'Parsed))) (HsRecFields GhcPs (Located (Pat (GhcPass 'Parsed)))))
...
-- -- deriving instance Show (FamEqn GhcRn (HsDataDefn GhcRn))
...
-- -- deriving instance Show (HsRecFields GhcRn (Located (Pat (GhcPass 'Renamed))))
-- -- deriving instance Show (HsConDetails (Located (Pat (GhcPass 'Renamed))) (HsRecFields GhcRn (Located (Pat (GhcPass 'Renamed)))))
...
-- -- deriving instance Show (HsArg (LHsType GhcRn) (LHsKind GhcRn))
-- -- deriving instance Show (StandaloneKindSig GhcRn)
-- -- deriving instance (Show a, Show b) => Show (FamEqn GhcRn a b)
-- -- deriving instance Show (FamEqn GhcRn (LHsType GhcRn))
...
-- -- deriving instance Show (FamEqn GhcTc (HsDataDefn GhcTc))
...
-- instance Show (VarEnv.TidyEnv -> TcM (VarEnv.TidyEnv, ErrUtils.MsgDoc))
-- instance Show a => Show (TcRef a)
-- -- deriving instance Show TcLclEnv
-- deriving instance Show InferResult
-- deriving instance Show TopLevelFlag
-- deriving instance Show ExpType
-- deriving instance Show UserTypeCtxt
-- -- deriving instance Show TcOrigin.SkolemInfo
-- -- deriving instance Show  Outputable.SDoc
-- deriving instance Show SpliceType
-- -- deriving instance Show Constraint.Implication
-- deriving instance Show TcBinder
-- -- deriving instance Show PendingStuff
-- -- deriving instance Show ThStage
-- -- deriving instance Show Constraint.WantedConstraints
-- instance Show LocalRdrEnv
-- instance Show (GHCi.RemoteTypes.ForeignRef (Language.Haskell.TH.Syntax.Q ()))
-- -- deriving instance Show Constraint.CtEvidence
-- -- deriving instance Show Ct
-- -- deriving instance Show ArrowCtxt
-- deriving instance Show TcType.TcLevel
...
-- deriving instance Show (ImportDeclQualifiedStyle)
...
-- -- deriving instance Show (HsRecFields GhcTc (Located (Pat (GhcPass 'Typechecked))))
-- -- deriving instance Show (HsConDetails (Located (Pat (GhcPass 'Typechecked))) (HsRecFields GhcTc (Located (Pat (GhcPass 'Typechecked)))))
...
-- -- deriving instance Show (StandaloneKindSig GhcTc)
-- -- deriving instance Show (TyClGroup GhcTc)
-- -- deriving instance Show (HsArg (LHsType GhcTc) (LHsKind GhcTc))
-- -- deriving instance Show (FamEqn GhcTc (LHsType GhcTc))
...
--   show = showSDocUnsafe . ppr
-- instance (OutputableBndrId p) => Show (HsIPBinds (GhcPass p)) where
--   show = showSDocUnsafe . ppr
